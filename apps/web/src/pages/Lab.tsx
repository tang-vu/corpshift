...
const STEPS = [
  { key: "seed", label: "Seed positions", desc: "deposit 10 stock tokens + borrow $400 in both vaults" },
...
function VaultPanel({ name, tag, tone, v, dead }: {
  name: string;
  tag: string;
  tone: "naive" | "aware";
  v: VaultView;
  ...
  return (
    <div className={`relative rounded-lg border ${accent} bg-panel-2 p-4 ${deadState ? "opacity-70" : ""}`}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-fg-faint">health factor</div>
          <div data-testid={`hf-${tone}`} className={`mt-1 font-mono text-2xl font-bold ${hfTone(v.healthFactor)}`}>{hf}</div>
        </div>
      </div>
    </div>
  );
...
export function Lab() {
  ...
  const refresh = useCallback(() => {
    api.demoState().then((s) => mounted.current && (setState(s), setErr(null))).catch((e) => mounted.current && setErr(e.message));
  }, []);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-fg-dim">
            Two identical lending vaults. One reads raw balances. One reads CorpShift.
            Same user, same collateral, same 4:1 split — <strong className="text-fg">diametrically opposed outcomes</strong>,
        ...
        <div className="flex gap-2">
          ...
          >
            {busy ? "executing…" : state.step >= STEPS.length ? "scenario complete" : `▶ ${nextLabel}`}
          </button>
          <button
      ...
      <Card>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
          <div>
            <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-fg-faint">asset state</div>
            <div className="mt-1"><StateBadge state={state.assetState} /></div>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div>
          <VaultPanel name="NaiveVault" tag="reads raw ERC-20 balances" tone="naive" v={state.vaults.naive} />
        ...
        <div>
          <VaultPanel name="CorpShiftAwareVault" tag="reads CorpShift economic units" tone="aware" v={state.vaults.aware} />
          <p className="mt-2 px-1 font-mono text-[11px] leading-relaxed text-fg-faint">
            ...
            {log.map((r, i) => (
              <div key={i} className="rounded-md border border-edge bg-panel-2 p-3">
                <div className="flex items-center gap-2">
                  <span className={`font-mono text-[11px] font-bold ${r.ok ? "text-green" : "text-amber"}`}>
                    {r.ok ? "✓" : "!"}
                  </span>
                  <span className="font-mono text-[12px] font-semibold uppercase tracking-wide">{r.step}</span>
                </div>
                  ...
                  <div className="mt-2 space-y-1">
                    {r.txs.map((t, j) => (
                      <div key={j} className="flex items-center gap-2 font-mono text-[11px]">
                        ...
                        <span className="ml-auto"><HexLink hex={t.hash} url={t.url} /></span>
                      </div>
                    ))}
                  </div>
                )}
